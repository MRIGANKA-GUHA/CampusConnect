...
import { ArrowLeft, Mail, Lock, User, Eye, EyeOff, CheckCircle2, Loader2, AlertCircle } from 'lucide-react';
...
export default function RegisterPage() {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      // You can store accountType/roll in a DB here if needed
      navigate('/dashboard');
    } catch (err) {
    ...
    <div className="relative min-h-screen w-full overflow-hidden font-sans flex items-center justify-center py-12">
      ...
      <div className="relative z-10 w-full max-w-[560px] xl:max-w-[640px] mx-4 sm:mx-auto">
        ...
        <div className="relative rounded-3xl bg-white/70 dark:bg-[#0e0e14]/80 backdrop-blur-2xl border border-white/80 dark:border-white/10 p-8 sm:p-12 shadow-2xl shadow-indigo-500/10 dark:shadow-black/50">
          ...
          {error && (
            <div className="mb-6 p-4 rounded-2xl bg-red-50 dark:bg-red-500/10 border border-red-100 dark:border-red-500/20 flex items-center gap-3 text-red-600 dark:text-red-400 text-sm">
          ...
          <form onSubmit={handleRegister} className="flex flex-col gap-5">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                  ...
                  className={`relative flex flex-col items-start p-4 pr-10 border-2 rounded-2xl transition-all ${accountType === 'student' ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-500/10' : 'border-slate-200 dark:border-white/8 bg-white/60 dark:bg-white/5 hover:border-indigo-300 dark:hover:border-indigo-500/50'}`}
                >
                  ...
                  {accountType === 'student' && <CheckCircle2 className="absolute top-1/2 -translate-y-1/2 right-4 w-4 h-4 text-indigo-600 dark:text-indigo-400" />}
                </button>

                <button
                  type="button"
                  onClick={() => setAccountType('convenor')}
                  className={`relative flex flex-col items-start p-4 pr-10 border-2 rounded-2xl transition-all ${accountType === 'convenor' ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-500/10' : 'border-slate-200 dark:border-white/8 bg-white/60 dark:bg-white/5 hover:border-indigo-300 dark:hover:border-indigo-500/50'}`}
                >
                  ...
                  {accountType === 'convenor' && <CheckCircle2 className="absolute top-1/2 -translate-y-1/2 right-4 w-4 h-4 text-indigo-600 dark:text-indigo-400" />}
                </button>
              </div>
            </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="mt-2 w-full py-4 rounded-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold tracking-wide shadow-lg shadow-indigo-500/20 hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2 disabled:opacity-70 disabled:hover:translate-y-0"
            >
            ...
            <div className="grid grid-cols-2 gap-3">
              <button 
                ...
                className="flex items-center justify-center gap-2 py-3 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/8 rounded-full hover:bg-slate-100 dark:hover:bg-white/10 transition-colors text-sm font-semibold text-slate-700 dark:text-slate-300 disabled:opacity-50"
              >
