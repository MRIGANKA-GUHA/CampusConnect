...
/**
 * ClubHeader — Navigation bar exclusively for club account pages.
 * Shows: Dashboard | Events | Notices | Members | Profile
 */
export default function ClubHeader() {
  ...
  const dashboardPath = '/club/dashboard';

  const navLinks = [
    { label: 'Events',  href: '/club/events',  icon: Calendar },
    { label: 'Notices', href: '/club/notices', icon: Bell },
    { label: 'Members', href: '/club/members', icon: Users },
  ];
          ...
          <div className="flex items-center gap-1 sm:gap-3">
            ...
            {user && (
              <Link to="/club/profile" className="w-8 h-8 sm:w-10 sm:h-10 rounded-full border-[1.5px] sm:border-2 border-indigo-200 dark:border-indigo-500/30 overflow-hidden shrink-0 shadow-sm cursor-pointer hover:border-indigo-500 dark:hover:border-indigo-400 hover:-translate-y-0.5 active:scale-95 transition-all bg-indigo-50 dark:bg-indigo-500/10" title="Club Profile">
                <img src={avatarSrc} alt="Club" className="w-full h-full object-cover" />
              </Link>
            )}
      ...
      {menuOpen && (
        <div className="fixed top-[4.5rem] left-3 right-3 z-40 bg-white dark:bg-zinc-950 border border-slate-200 dark:border-white/10 rounded-3xl shadow-xl p-5 flex flex-col gap-3 md:hidden">
          <Link
            ...
            className={`text-base px-2 py-2.5 rounded-xl transition-all flex items-center gap-2 ${location.pathname === dashboardPath ? 'font-bold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-500/10' : 'font-semibold text-slate-700 dark:text-slate-200 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-slate-50 dark:hover:bg-white/5'}`}
          >
          ...
          {navLinks.map(({ label, href, icon: Icon }) => {
            ...
            return (
              <Link
                ...
                to={href}
                onClick={() => setMenuOpen(false)}
                className={`text-base px-2 py-2.5 rounded-xl transition-all flex items-center gap-2 ${isActive ? 'font-bold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-500/10' : 'font-semibold text-slate-700 dark:text-slate-200 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-slate-50 dark:hover:bg-white/5'}`}
              >
